...
| 13 | `data_protection.intake_provider` | **Added 17 Sep 2026; rewritten 19 Sep 2026.** The form service is now **Netlify, Inc.**, which hosts the site and receives Practice Discovery responses as processor. Its entity name, processor role, DPA and transfer clauses are established from its own documents and are already written into the page. What remains: confirm the DPA has been read and applies to this account, and record the plan and account name. | Privacy §3 and "Who else handles your information" |
| 14 | `data_protection.intake_retention` | **Added 17 Sep 2026; rewritten 19 Sep 2026.** How long Practice Discovery **text responses** stay in Netlify, as actually operated. No uploads exist to retain. Netlify's Forms documentation does not state an automatic deletion schedule for form responses, so this is entirely operational practice. | Privacy §3 and retention list |
...
| Privacy notice §3 and processors | **Rewritten 19 September 2026.** Netlify, Inc. named as the processor for Practice Discovery responses; the statement that no files are uploaded through the questionnaire; the statement that responses may be processed outside the UK, sourced to Netlify's privacy statement that personal data may be transferred to and stored outside the country of collection; reliance on Netlify's DPA in the order its clause 14 sets: the UK Extension to the EU–US Data Privacy Framework first, with the EU Standard Contractual Clauses as amended by the UK International Data Transfer Addendum as the stated fallback if the Framework is invalidated or Netlify fails to re-certify | Is reliance on the UK Extension to the Data Privacy Framework, with the clauses behind it, acceptable here — given that Netlify's published documents do not commit to a UK or EEA storage region, the responses are business rather than special-category data, and the notice deliberately describes the arrangement without asserting that it is legally sufficient? Does the "no files through the form" statement need anything added about the shared-folder route, where the Studio is not the controller of the client's own storage account? |
