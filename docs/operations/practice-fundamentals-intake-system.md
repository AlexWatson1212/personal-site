...
```mermaid
flowchart TD
  A[Enquiry +<br/>5-question fit check] --> B[£100 deposit]
  B --> C[Welcome email<br/>intake + photo brief]
  C --> D[Intake form<br/>photos arranged in parallel]
  D --> E{Enough to start?}
  E -- no --> F[One clarification<br/>message or 20-min call]
  F --> E
  E -- yes --> G[Practice Fundamentals<br/>facts table + live preview]
  G --> H[Client reply: facts +<br/>feedback stage 1]
  H --> I[£395 balance paid]
  I --> J[Build from the<br/>approved preview]
  J --> K[Client feedback<br/>stage 2]
  K --> L[Corrections,<br/>domain, launch]
```
